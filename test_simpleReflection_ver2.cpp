...
void test_simple() {
  constexpr size_t count = binary_search_member_field<simple>();
  static_assert(count == 2, "count is not 2");
  std::cout << "test_simple passed!" << std::endl;
}

void test_nested() {
  constexpr size_t count = binary_search_member_field<nested>();
  assert(count == 2);
  std::cout << "test_nested passed!" << std::endl;
}

void test_array_struct() {
  constexpr size_t count = binary_search_member_field<array_struct>();
  assert(count == 2);
  std::cout << "test_array_struct passed!" << std::endl;
}

void test_complex() {
  constexpr size_t count = binary_search_member_field<complex>();
  assert(count == 3);
